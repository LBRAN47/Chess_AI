...
class Parser():
    ...
    def get_coordinate(self, row, col):
        ...
        if col not in self.COLUMNS or row not in self.ROWS:
            ...
            return
        return (self.COLUMN_CONVERT[col], self.ROW_CONVERT[row])
    ...
    def parse_move(self, move, board):
        ...
        if move[0] in self.COLUMNS:
            ...
            row, col = None, None
            if move[0] in self.ROWS:
                row, move = self.ROW_CONVERT[move[0]], move[1:] 
            ...
            if len(move) < 2:
                ...
                return
            target_coords = self.convert_coordinate(move[:2])
            move = move[2:]
            if len(move) == 0:
